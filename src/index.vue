<template>
  ...
  <template v-if="computedConfig.serverData">
    <template v-if="locale === 'de'">
      <h4>Serverdaten</h4>
      <p>
  ...
  <template v-if="computedConfig.cookies">
    <template v-if="locale === 'de'">
      <h4>Cookies</h4>
  ...
  <template v-if="computedConfig.contact">
    <template v-if="locale === 'de'">
  ...
  <template v-if="computedConfig.registration">
    <template v-if="locale === 'de'">
  ...
  <template v-if="computedConfig.firebaseAuthentication">
    <h4>Firebase Authentication</h4>
    <template v-if="locale === 'de'">
      <p>
        Firebase Authentication ist ein von Google Ireland Limited
        bereitgestellter Registrierungs- und Anmeldedienst. Um den Anmelde- und
        Anmeldeprozess zu vereinfachen, kann Firebase Authentication
        Identitätsdienste von Drittparteien benutzen und die Informationen auf
        ihrer Plattform speichern.
      </p>
      <p>
        Verarbeitete personenbezogene Daten: E-Mail; Konten in den sozialen
        Netzwerken; Nachname; Nutzername; Passwort; Profilbild; Telefonnummer;
        Vorname.
      </p>
      ...
      <p>
        Rechtsgrundlage für die Übermittlung der Daten außerhalb der EU: Von der
        EU Kommission erlassene Standarddatenschutzklauseln.
      </p>
      <p>Speicherdauer: Aufbewahrung der Daten für die Dauer des Vertrages.</p>
    </template>
    <template v-else>
      <p>
        Firebase Authentication is a registration and authentication service
        provided by Google LLC or by Google Ireland Limited, depending on the
        location Strive is accessed from. To simplify the registration and
        authentication process, Firebase Authentication can make use of
        third-party identity providers and save the information on its platform.
      </p>
  ...
  <template v-if="computedConfig.newsletter">
    <h4>Newsletter</h4>
    <template v-if="locale === 'de'">
      <p>
  ...
  <template v-if="computedConfig.firebaseCloudFirestore">
    <h4>Firebase Cloud Firestore</h4>
    <template v-if="locale === 'de'">
      <p>
        Firebase Cloud Firestore ist ein Webhosting und Backend Dienst,
        bereitgestellt von Google Ireland Limited.
      </p>
      <p>
    ...
    <template v-else>
      <p>
        ...
        Google LLC or by Google Ireland Limited, depending on the location
        Strive is accessed from.
      </p>
      <p>
        Personal Data processed: Usage Data; various types of Data as specified
  ...
  <template v-if="computedConfig.firebaseCloudStorage">
    ...
    <template v-else>
      <p>
        Firebase Cloud Storage is a hosting service provided by Google LLC or by
        Google Ireland Limited, depending on the location Strive is accessed
        from.
      </p>
      <p>
        Personal Data processed: Usage Data; various types of Data as specified
  ...
  <template v-if="computedConfig.firebaseCloudFunctions">
  ...
  <template v-if="computedConfig.googleAnalytics">
  ...
  <template v-if="computedConfig.firebaseAnalytics">
    <h4>Google Analytics for Firebase</h4>
    <template v-if="locale === 'de'">
      <p>
        Google Analytics für Firebase oder Firebase Analytics ist ein
        Analyse-Dienst von Google Ireland Limited.
      </p>
      <p>
        Weitere Informationen zur Verwendung von Daten bei Google sind in der
        Partner-Richtlinie von Google einsehbar. Firebase Analytics kann Daten
        mit anderen von Firebase bereitgestellten Tools wie Crash Reporting,
        Authentication, Remote Config oder Notifications gemeinsam nutzen. Der
        Nutzer kann diese Datenschutzerklärung überprüfen, um eine ausführliche
        Erläuterung zu den anderen vom Eigentümer verwendeten Tools zu finden.
        Diese Website verwendet Identifikatoren für mobile Geräte und
        Cookie-ähnliche Technologien für die Ausführung des Google Analytics for
        Firebase-Dienstes. Nutzer können sich über die entsprechenden
        Geräteeinstellungen mobiler Geräte von bestimmten Firebase-Funktionen
        abmelden, wie etwa über die Werbeeinstellungen für mobile Geräte, oder
        indem sie gegebenenfalls den Anweisungen anderer Abschnitte dieser
        Datenschutzrichtlinie bezüglich Firebase folgen.
      </p>
      <p>
        Verarbeitete personenbezogene Daten: Anwendung-Ausführungen;
        Anwendung-Updates; Anzahl der Nutzer; Anzahl der Sitzungen; App-Starts;
        Betriebssysteme; Eindeutige Gerätekennzeichnung für Werbung
        (Google-Werbe-ID oder IDFA, beispielsweise); Geräteinformationen;
        In-App-Käufe; Nutzungsdaten; Region; Sitzungsdauer; Tracker.
      </p>
      <p>
        Verarbeitungsort: Irland –
        <a href="https://policies.google.com/privacy" target="_blank"
    ...
    <template v-else>
      <p>
        ...
        service provided by Google LLC or by Google Ireland Limited, depending
        on the location Strive is accessed from.
      </p>
      <p>
        ...
        explanation about the other tools used by the Owner. Strive uses
        identifiers for mobile devices and technologies similar to cookies to
        run the Firebase Analytics service. Users may opt-out of certain
        Firebase features through applicable device settings, such as the device
        advertising settings for mobile phones or by following the instructions
        in other Firebase related sections of this privacy policy, if available.
      </p>
      <p>
        Personal Data processed: Application opens; Application updates; device
        information; geography/region; In-app purchases; launches; number of
        sessions; operating systems; session duration; Tracker; unique device
      ...
      <p>
        Category of personal information collected according to CCPA:
        identifiers; commercial information; internet information; geolocation
        data.
      </p>
      <!-- https://www.striveworkouts.com/privacy/ -->
    </template>
  ...
  <template
    v-if="
      computedConfig.googleTagManager ||
      computedConfig.googleAnalytics ||
      computedConfig.firebaseAnalytics
    "
  >
    <h4>Google Tag Manager</h4>
    <template v-if="locale === 'de'">
      <p>
        Google Tag Manager ist ein von Google Ireland Limited bereitgestellter
        Dienst zur Verwaltung von Tags.
      </p>
    ...
    <template v-else>
      <p>
        Google Tag Manager is a service provided by Google LLC or by Google
        Ireland Limited to manage tags.
      </p>
  ...
  <template v-if="computedConfig.matomo">
    <template v-if="locale === 'de'">
      ...
      <p>
        ...
        >
        sein. Unser berechtigtes Interesse liegt in der Analyse und Optimierung
    ...
    <template v-else>
      ...
      <p>
        In case you have granted your consent to this processing the legal basis
        is Article 6 para. 1 lit. a GDPR. The legal basis can also be Article 6
        para. 1 lit. f GDPR. Our legitimate interest lies in the analysis and
  ...
  <p v-else>
    ...
    >
    for
    <a href="https://www.ratgeberrecht.eu/" rel="noopener" target="_blank"
      >Anwaltskanzlei Weiß &amp; Partner</a
    >
  </p>
</template>
